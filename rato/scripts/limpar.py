#!/usr/bin/env python3
"""
limpar.py — limpeza unificada de Markdown extraído de PDF para a biblioteca do rato.
...
Uso:
    ...
    python3 rato/scripts/limpar.py --pasta . --in-place
    python3 rato/scripts/limpar.py arquivo.md --in-place --com-ollama
    python3 rato/scripts/limpar.py arquivo.md --in-place --marcar-duvidas
...
def converter_notas_markdown(texto: str) -> str:
    ...
    # Encontrar definições explícitas com os mesmos padrões flexíveis
    # do antigo converter_notas.py.
...
def catalogar_saida(
    ...
    """Aplica a etapa de catalogacao bibliografica no arquivo ja limpo."""
    from catalogar import (
        carregar_autores,
        catalogar_arquivo,
        escrever_com_frontmatter,
        salvar_backup,
    )

    autores_simples, autores_compostos = carregar_autores(autores)
    ...
    if not sem_backup:
        salvar_backup(caminho, Path("rato/backups/catalogar"))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="limpar.py — limpeza unificada de Markdown extraído de PDF para a biblioteca do rato",
        epilog=__doc__,
    ...
    parser.add_argument("--catalogar", action="store_true", help="Depois da limpeza, adiciona/atualiza frontmatter bibliografico")
    parser.add_argument("--catalogar-forcar", action="store_true", help="Com --catalogar, recalcula metadados existentes")
    parser.add_argument("--catalogar-sem-backup", action="store_true", help="Com --catalogar, nao cria backup da catalogacao")
    parser.add_argument("--autores", type=Path, default=None, help="Com --catalogar, caminho para autores.yaml")
    parser.add_argument(
        "--limpeza-pdf",
        action="store_true",
        help="Compatibilidade: a limpeza estrutural de PDF já faz parte do pipeline padrão",
    )
    ...
    if args.pasta:
        ...
        for arq in arquivos:
            try:
                ...
                msg_catalogar = ""
                if args.catalogar:
                    alterados = catalogar_saida(
                        saida,
                        forcar=args.catalogar_forcar,
                        sem_backup=args.catalogar_sem_backup,
                        autores=args.autores,
                    )
                    msg_catalogar = (
                        f" | catalogado: {', '.join(alterados)}"
                        if alterados else " | catalogado: sem alterações"
                    )
                print(f"  ✅ {arq.name} → {saida.name}{msg_catalogar}")
                ok += 1
        ...
        if args.catalogar:
            ...
            if alterados:
                print(f"✅ {args.arquivo.name} → {saida.name} | catalogado: {', '.join(alterados)}")
            else:
                print(f"✅ {args.arquivo.name} → {saida.name} | catalogado: sem alterações")
        else:
