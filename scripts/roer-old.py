...
TIMEOUT_SEGUNDOS = 180           # timeout por chamada ao Ollama
...
Você está ajudando a construir uma biblioteca acadêmica em Obsidian.

Leia o trecho abaixo com calma e produza notas úteis para uma revisão de literatura.
O objetivo é ser fiel ao trecho, não demonstrar erudição externa.
Este é o trecho {parte_atual} de {total_partes} do documento "{titulo}".

ORIENTAÇÕES DE LEITURA:
- Use apenas o que aparece neste trecho.
- Quando algo não estiver claro, escreva "não identificado neste trecho".
- Preserve conceitos nomeados pelo autor, especialmente expressões originais em inglês.
- Preserve termos técnicos em inglês quando a tradução literal puder deformar o sentido: stochasticity, niche construction, feedback, Umwelt, affordances, algorithm, collective behavior.
- Se traduzir um termo técnico, mantenha também o original entre parênteses.
- Separe com cuidado argumento, exemplo, referência, metáfora, citação e metadado editorial.
- Se o trecho for agradecimento, bibliografia, referências, notas editoriais, ficha catalográfica ou lista de links, classifique como MATERIAL PARATEXTUAL e não extraia argumento do autor.
- Não trate todos os trechos como igualmente densos: identifique se o trecho contém argumento central, exemplo, transição, contextualização ou apenas paratexto.
- Prefira frases concretas a avaliações genéricas.
- Se houver vários eixos no trecho, mantenha essa multiplicidade.
- Indique a base textual usando [Parte {parte_atual}].
- Se fizer uma ponte interpretativa para minha pesquisa, marque como "inferência limitada".
...
TIPO-DE-TRECHO:
<classifique como argumento central, exemplo, transição, contextualização, método, discussão conceitual, conclusão, ou MATERIAL PARATEXTUAL>

DENSIDADE-ARGUMENTATIVA:
<alta, média, baixa ou paratextual; justifique em uma frase com [Parte {parte_atual}]>

RESUMO-FIEL:
<resumo específico do trecho, em 1 a 3 parágrafos curtos, com [Parte {parte_atual}]>

ARGUMENTO-DO-AUTOR:
<o que o autor efetivamente sustenta neste trecho; se não houver argumento, escreva "não identificado neste trecho"; se for MATERIAL PARATEXTUAL, escreva "não aplicável: material paratextual">

CONCEITOS:
<conceitos nomeados ou expressões importantes do trecho; mantenha termos originais quando forem relevantes>

OPERADORES-CENTRAIS:
<conceitos que organizam relações entre outros conceitos; se não houver, escreva "não identificado neste trecho"; não confunda tema recorrente com operador>

TENSOES-ESTRUTURANTES:
<dualidades, polaridades ou tensões que organizam o argumento; se não houver, escreva "não identificado neste trecho">

FRASES-CHARNEIRA:
<até 3 frases curtas do trecho que reorganizam o argumento; se não houver, escreva "não identificado neste trecho">

EXEMPLOS:
<casos, imagens, objetos, tecnologias ou situações mencionadas no trecho; se não houver, escreva "não identificado neste trecho">

AUTORES-OBRAS:
<autores e obras citados no trecho; se não houver, escreva "não identificado neste trecho">

EVIDENCIAS:
<3 a 6 pontos verificáveis do trecho, cada um começando com [Parte {parte_atual}]; se for MATERIAL PARATEXTUAL, registre apenas o tipo de material e não invente evidências conceituais>

DESTAQUES-INTERPRETATIVOS:
Escolha no máximo 2 pontos do trecho que merecem atenção. Para cada ponto, use:
- Ponto: <título curto>
- Base: [Parte {parte_atual}]
- Por que importa: <explique brevemente>
- Interpretação limitada: <uma leitura possível, sem transformar em certeza>
- Dúvida aberta: <o que ainda não está claro>

PALAVRAS-CHAVE:
<até 8 termos específicos separados por vírgula, sem numeração>
...
Você está consolidando notas de leitura de um texto chamado "{titulo}".

Abaixo estão unidades de leitura do escopo: {escopo}.

Sua tarefa é produzir um RESUMO INTERMEDIÁRIO compacto e fiel, em português, que:
- Preserve os conceitos nomeados pelo autor, especialmente expressões originais
- Preserve termos técnicos em inglês quando forem centrais
- Identifique os argumentos centrais dessas partes
- Diferencie argumento central, exemplo, transição e MATERIAL PARATEXTUAL
- Liste autores e obras citados nessas partes
- Mantenha as distinções importantes sem colapsá-las
- Não avalie nem interprete; apenas sintetize fielmente
- Preserve as referências [Parte N] que vierem das notas
- Seja curto o bastante para entrar numa próxima rodada feita por modelo local.

Formato obrigatório:

PARTES-COBERTAS:
<liste todas as referências [Parte N] cobertas por esta síntese>

MAPA-DE-DENSIDADE:
<indique quais partes têm argumento central, quais são exemplos, quais são transições e quais são MATERIAL PARATEXTUAL>

RESUMO-INTERMEDIARIO:
<síntese compacta em 2 parágrafos curtos, com referências [Parte N]>

CONCEITOS-INTERMEDIARIOS:
<até 8 conceitos e termos relevantes dessas partes>

OPERADORES-INTERMEDIARIOS:
<até 6 operadores relacionais que organizam essas partes>

TENSOES-INTERMEDIARIAS:
<até 6 tensões, polaridades ou dualidades estruturantes dessas partes>

AUTORES-INTERMEDIARIOS:
<até 8 autores e obras citados nessas partes>

PALAVRAS-CHAVE-INTERMEDIARIAS:
<até 8 termos separados por vírgula>
...
Você está produzindo a CAMADA 2 — ESTRUTURA DO TEXTO — para uma ficha de leitura.

Texto: "{titulo}".

Use somente as notas abaixo. Não use conhecimento externo.
Esta camada deve identificar a organização interna do texto, sem relacionar ainda com a pesquisa do leitor.
...
TESE-ESTRUTURAL:
<qual argumento organiza o texto inteiro, com referências [Parte N]>

CADEIA-ARGUMENTATIVA:
<liste de 4 a 8 passos mostrando como o texto progride, sempre com [Parte N]>

OPERADOR-PRINCIPAL:
<qual conceito organiza os demais, mesmo que não seja o mais frequente, com [Parte N]>

OPERADORES-SECUNDARIOS:
- <conceito> — <relações que organiza> [Parte N]

FRASES-CHARNEIRA:
- <frase curta do texto ou paráfrase muito próxima> — <por que reorganiza o argumento> [Parte N]

TENSOES-ESTRUTURANTES:
- <polaridade ou tensão> — <como estrutura o argumento> [Parte N]

FIGURAS-ORGANIZADORAS:
<mitos, imagens, objetos, cenas, materiais ou processos que organizam o texto; se não houver, escreva "não identificado nas notas">

REGIME-DO-TEXTO:
<indique se o texto opera sobretudo em regime simbólico, fenomenológico, técnico, historiográfico, filosófico, metodológico, empírico, quantitativo ou outro; justifique com [Parte N]>
...
PROMPT_RESSONANCIAS = """\
Você está produzindo a CAMADA 3 — RESSONÂNCIAS COM A PESQUISA DO LEITOR.

Texto: "{titulo}".

Use as notas do texto e a memória conceitual abaixo.
Diferencie rigorosamente o que vem do texto e o que é inferência do leitor.
Não atribua ao autor conceitos que aparecem apenas na memória conceitual.
Toda hipótese precisa indicar GRAU-DE-CONFIANÇA.
...
CONCEITOS-DO-TEXTO:
- <conceito> — origem: autor/texto — base: [Parte N]

CONCEITOS-DO-LEITOR-QUE-RESSOAM:
- <conceito da memória> — origem: leitor/memória — relação possível com o texto: <explique> — GRAU-DE-CONFIANÇA: <baixo/médio/alto>

HIPOTESES-DE-APROXIMACAO:
- Hipótese: <aproximação possível entre texto e pesquisa do leitor> [Parte N]
  Origem: inferência do leitor
  GRAU-DE-CONFIANÇA: <baixo/médio/alto>

LIMITES-DA-APROXIMACAO:
- <onde a aproximação pode forçar o texto ou produzir distorção>

PERGUNTAS-GERADAS:
- <pergunta situada que emerge do encontro entre texto e pesquisa>
...
TAREFA:
Use somente as notas abaixo. Não use conhecimento externo.
Use também a CAMADA 2 e a CAMADA 3 fornecidas abaixo, respeitando suas distinções entre texto, estrutura e inferência do leitor.
Comece diretamente por **Tese central**.
Mantenha exatamente os títulos do molde.
Não escreva "Resumo", "Análise", "Conclusão" nem títulos com ###.
Toda afirmação substantiva precisa de referência [Parte N].
As seções até **Contribuição para o campo** devem usar apenas o que o texto sustenta.
Não use conceitos da memória conceitual para formular a tese, o deslocamento teórico ou a contribuição do texto.
Use a CAMADA 2 para formular operador principal, cadeia argumentativa, frases-charneira, tensões e figuras organizadoras; use a CAMADA 3 somente em **Possíveis relações com minha pesquisa**.
Só use a memória conceitual na seção **Possíveis relações com minha pesquisa**.
Cubra início, meio e fim das notas; não concentre a ficha apenas nas últimas partes.
Em textos com muitas partes, use ao menos uma referência do início, uma do meio e uma do fim.
Use os campos PARTES-COBERTAS como mapa para distribuir a leitura.
Relações com minha pesquisa devem ser hipóteses, com GRAU-DE-CONFIANÇA.
Se algo não estiver claro nas notas, escreva "não identificado nas notas".
Não force relações.

**Tese central**
<um parágrafo sobre o argumento principal, com referências [Parte N]>

**Deslocamento teórico**
<um parágrafo sobre a mudança de perspectiva proposta pelo texto, com referências [Parte N]>

**Conceitos-chave do autor**
- <conceito ou expressão> — <explicação breve> [Parte N]

**Operadores centrais**
- <conceito que organiza relações> — <que relações ele organiza> [Parte N]

**Frases-charneira**
- <frase curta do texto ou paráfrase muito próxima> — <por que reorganiza o argumento> [Parte N]

**Figuras organizadoras**
- <mito, imagem, objeto, cena, material ou processo que organiza o texto> — <como atua no argumento; se não houver, escreva "não identificado nas notas"> [Parte N]

**Cadeia argumentativa**
- <passo do argumento> [Parte N]

**Tensões estruturantes**
- <polaridade ou tensão> — <como estrutura o argumento> [Parte N]

**Exemplos empíricos**
- <caso, imagem, tecnologia ou situação> [Parte N]

**Autores e referências mobilizadas**
- <autor ou obra> [Parte N]

**Contribuição para o campo**
<um parágrafo, sem extrapolar além das notas, com referências [Parte N]>

**Possíveis relações com minha pesquisa**
- Hipótese: <relação possível com matéria, cuidado, precariedade, reparo, manutenção, gesto, técnica ou infraestruturas> [Parte N]
  GRAU-DE-CONFIANÇA: <baixo / médio / alto>
...
<como o texto poderia ser reduzido ou mal interpretado, com referências [Parte N]>

**Evidências para conferência**
- <afirmação verificável> [Parte N]

**O que esta leitura pode ter distorcido**
<onde a ficha pode ter simplificado, deslocado ou perdido nuance em relação ao texto original; se não houver distorção identificada, escreva "não identificado">

PALAVRAS-CHAVE-FINAIS:
<até 12 termos específicos separados por vírgula, sem numeração>
...
def consolidar_resumos(
    ...
    resposta = chamar_ollama(prompt, modelo, num_predict=3500, num_ctx=16384)
    ...
    if not ficha_valida(ficha, total_partes=len(leituras)):
        ...
        resposta = chamar_ollama(prompt_retry, modelo, num_predict=3500, num_ctx=16384)
        ficha, palavras_chave = separar_ficha_e_palavras_chave(resposta)
